...
;;;; Extensible Looping Macros, version 9
...
(define (%cars&cdrs lists cars-tail cdrs-tail)
  (loop proceed ((for list (in-list lists))
    ...
    (if (pair? list)
        ...
        (values #t #f #f))))

;;;;; List Structure Iteration

;;; (FOR <pair> (ON-LIST <list> [<successor>]))
;;; (FOR (<element> ...) [<pair>] (ON-LIST <list> [<successor>]))

(define-syntax on-list
  (syntax-rules ()
    ((ON-LIST ((element-variable ...) pair-variable)
              (list-expression successor-expression)
              next . rest)
     (next (((LIST) list-expression)           ;Outer bindings
            ((SUCCESSOR) successor-expression))
           ...
           (((element-variable ...)           ;Body bindings
             (%ON-LIST:TAKE-VALUES pair-variable (element-variable ...)))
            ((TAIL)
             (SUCCESSOR pair-variable)))
           ()                                 ;Final bindings
           . rest))

    ((ON-LIST ((element-variable ...))
              (list-expression successor-expression)
              next . rest)
     (ON-LIST ((element-variable ...) PAIR)
              (list-expression successor-expression)
              next . rest))
    ...
    ((ON-LIST ((element-variable ...)) (list-expression) next . rest)
     (ON-LIST ((element-variable ...) PAIR) (list-expression CDR) next . rest))

    ((ON-LIST (pair-variable)
              (list-expression successor-expression)
              next . rest)
     (IN-LIST (ELEMENT pair-variable)
              (list-expression successor-expression)
              next . rest))

    ((ON-LIST (pair-variable) (list-expression) next . rest)
     (ON-LIST (pair-variable) (list-expression CDR) next . rest))

    ((ON-LIST variables arguments next . rest)
     (LOOP-CLAUSE-ERROR (ON-LIST variables arguments
                                 "Malformed ON-LIST clause in LOOP:")))))

